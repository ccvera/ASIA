...
class TemporalAttentionLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_score = torch.tanh(self.attention(lstm_out))
...
attention_dim = 64
output_dim = 1
model = TemporalAttentionLSTM(input_dim, hidden_dim, attention_dim, output_dim)
...
plt.ylabel('True')
plt.savefig('_images/confusion_matrix_talstm.png')
plt.show()

# Classification report
class_report = classification_report(y_true, y_pred_class, target_names=['No Rain', 'Rain'], digits=4)
...
plt.savefig('_images/roc_curve_talstm.png')
plt.show()

# Save the model
torch.save(model.state_dict(), '_modelos/talstm_model.pth')

# Save the scaler
joblib.dump(scaler, '_modelos/scaler_talstm.pkl')
